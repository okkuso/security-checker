...
function CTASection({ url, score, shareText, shareUrl }: { url: string; score: number; shareText: string; shareUrl: string }) {
      ...
      <div className="bg-zinc-900 text-white rounded-2xl p-8 mb-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            onClick={() => {
              if (navigator.share) {
                navigator.share({ title: "セキュリティ診断結果", text: shareText, url: shareUrl });
              } else {
...
function ResultContent() {
  ...
  const shareUrl = `${typeof window !== "undefined" ? window.location.origin : ""}/result?url=${encodeURIComponent(result.url)}&score=${result.score}`;
  ...
  return (
    <>
      ...
      {/* CTA Section */}
      <CTASection url={result.url} score={result.score} shareText={shareText} shareUrl={shareUrl} />
    </>
  );
}
